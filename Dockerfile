...
FROM --platform=$BUILDPLATFORM mcr.microsoft.com/dotnet/sdk:11.0-preview-alpine AS build-env
ARG TARGETARCH
WORKDIR /app
...
ENV HUSKY=0
RUN dotnet publish -f net11.0 -a $TARGETARCH -o /app/out/

FROM mcr.microsoft.com/dotnet/${RuntimeImageRepo}:11.0-preview-alpine AS final
