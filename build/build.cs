    ...
    .WithTask("test", b =>
    {
        b.WithDescription("dotnet test")
            ...
            .WithExecution(async cancellationToken =>
            {
                ...
                {
                    await ExecuteCommandAsync($"dotnet run --collect:\"XPlat Code Coverage;Format=cobertura,opencover;ExcludeByAttribute=ExcludeFromCodeCoverage,Obsolete,GeneratedCode,CompilerGenerated\" {project}", cancellationToken);
                }
            })
            ;
    })
